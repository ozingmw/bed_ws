...
def camera2world(depth_image):
    ...
    X_c = X_c_value * depth_image  # shape: (height, width)
    Y_c = Y_c_value * depth_image  # shape: (height, width)
    Z_c = depth_image  # 이미 depth가 있음

    # 카메라 좌표 포인트들의 배열 생성
    # camera_points = np.stack((X_c, Y_c, Z_c), axis=-1)  # 필요하지 않음
    ...
    Z_w[depth_image == 0] = np.nan  # 또는 다른 값을 지정하여 유효하지 않음을 표시
